...
public class B {
	...
	public void solve() throws Exception{
		...
		ans = getcnt(N-cnt1, d1, 1);
		ans = Math.min(ans, getcnt(N-cnt2, d2, 1));
		//ans = Math.min(ans, getcnt(N-cnt3, d3));
		println(ans);
	}
	
	int getcnt(int cnt, boolean[][] d, int prev){
		if(cnt == 0){
			return 0;
		}
		if(prev <= 1){
