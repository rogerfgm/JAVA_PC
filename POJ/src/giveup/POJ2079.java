...
//wa
// 凸包して２点固定のきゃりーぽくやればできるらしいが、waでわからず
public class POJ2079 {
	...
	public void solve() throws Exception{
		...
		for(int i = 0; i < size-2; i++){
			...
			for(int j = i+1; j < size-1; j++){
				...
				while(k < size-1 && area(cvx.get(i), cvx.get(j), cvx.get(k)) < area(cvx.get(i), cvx.get(j), cvx.get(k+1))){
					k++;
				}
				ans = max(ans, area(cvx.get(i), cvx.get(j), cvx.get(k)));
				k--;
			}
		}
		
		int sei = (int)ans;
		int shou = (int)(ans * 100 % 100);
