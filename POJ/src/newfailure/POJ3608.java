...
// porigon同士の距離。
// 一番近い点どうしで、その点とその両端の線との垂線の長さで出そうとして当然WA
public class POJ3608 {
	...
	public void solve() throws Exception{
		...
		{
			P p = new P(xn[n], yn[n]);
			L ml1 = new L(xm[m], ym[m], xm[(m+1)%M], ym[(m+1)%M]);
			L ml2 = new L(xm[m], ym[m], xm[(m-1 + M)%M], ym[(m-1+M)%M]);
			
			P p1 = projection(ml1, p);
			if(intersectSP(ml1, p1)){
				dist = min(dist, dist(p1, p));
			}
			P p2 = projection(ml2, p);
			if(intersectSP(ml2, p2)){
				dist = min(dist, dist(p2, p));
			}
		}
		{
			P p = new P(xm[m], ym[m]);
			L ml1 = new L(xn[n], yn[n], xn[(n+1)%N], yn[(n+1)%N]);
			L ml2 = new L(xn[n], yn[n], xn[(n-1 + N)%N], yn[(n-1+N)%N]);
	...
	double dist(int n, int m){
		return dist(xn[n], yn[n], xm[m], ym[m]);
	}
