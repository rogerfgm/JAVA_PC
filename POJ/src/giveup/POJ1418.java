...
// WA
// 交点が他の円の内側にあるかどうかでwaだった。
// ネット調べてないので後でしらべる
public class POJ1418 {

	static PrintStream out = null;
	int INF = Integer.MAX_VALUE / 10;
	static Scanner sc = null;
	double DF = 0.00000000000001;
...
//						for(int k = i+1; k < N; k++){
//							if(k == j) continue;
//							double tmpd = dist(cx, cy, cs[k].p.x, cs[k].p.y);
//							if(dist(cx, cy, cs[k].p.x, cs[k].p.y) + DF <= cs[k].r){
//								tf = false;
//							}
//						}
//						if(tf){
...
//				}
			...
			for(int j = i+1; j < N-1; j++){
				for(int k = j+1; k < N; k++){
					List<P> ps = cc_cross(cs[j], cs[k]);
					for(P p : ps){
						if(c.contain(p)){
							list.add(new Data(j, k, p));
						}
					}
				}
			}
