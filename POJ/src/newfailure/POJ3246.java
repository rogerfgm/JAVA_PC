...
public class POJ3246 {
	...
	public void solve() throws Exception{
		...
		for(int i = 0; i < N; i++){
			...
			int p2 = (i+1) % N;
			int p3 = (i+2) % N;
			double len1 = len(dx[p1], dy[p1], dx[p2], dy[p2]);
			double len2 = len(dx[p3], dy[p3], dx[p2], dy[p2]);
			double len3 = len(dx[p1], dy[p1], dx[p3], dy[p3]);
			double men = menseki3Hen(len1, len2, len3);
			max = max(max, men);
		}
		double ans = area - max;
		int sei = (int)ans;
		int shou = (int)(ans * 100) % 100;
		out.println(sei + "." + shou);
